...
import org.farm.fireflyserver.common.exception.UnauthorizedException;
...
public class ExceptionHandlerFilter extends OncePerRequestFilter {
    ...
    private void handleUnauthorizedException(HttpServletResponse response, Exception e) throws IOException {
        ...
        if (e instanceof UnauthorizedException ue) {
            ...
            response.getWriter().write(objectMapper.writeValueAsString(BaseResponse.of(ue.getErrorCode())));
        } else if (e instanceof InvalidValueException ie) {
            response.setStatus(ie.getErrorCode().getHttpStatus().value());
            response.getWriter().write(objectMapper.writeValueAsString(BaseResponse.of(ie.getErrorCode())));
        }
    }
    ...
    private void handleException(HttpServletResponse response) throws IOException {
        ...
        response.getWriter().write(objectMapper.writeValueAsString(BaseResponse.of(ErrorCode.INTERNAL_SERVER_ERROR)));
    }
}